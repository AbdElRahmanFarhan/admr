function OptimalPath=ASTARPATH(StartX,StartY,MAP,GoalRegister,Connecting_Distance,physical_constraints)
%Version 1.0
% By Einar Ueland 2nd of May, 2016

%FINDING ASTAR PATH IN AN OCCUPANCY GRID

test = physical_constraints*2/3;
test2 = round(test);
%nNeighboor=3;
        ...
        if(ClosedMAT(CurrentY+i,CurrentX+j)==0) %Neiboor is open;
            if (abs(i)>=1||abs(j)>=1);   
                ...
                for K=1:JumpCells
                    ...
                    %end
                    for c=0:test2
                        if((CurrentY+YPOS+c>735)||(CurrentX+XPOS+c>1309)||(CurrentY-YPOS-c<1)||(CurrentX-XPOS-c<1)||...
                                (CurrentX-XPOS-c>1309)||(CurrentY-YPOS-c>735))
                            continue;
                            else
                                if (MAP(CurrentY+YPOS+c,CurrentX+XPOS+c)==1)
                                    Flag=0;
                                end
                                if (MAP(CurrentY-YPOS-c,CurrentX-XPOS-c)==1)
                                    Flag=0;
